...
class OccNet(BEVDepth):
    def __init__(
        ...
        **kwargs,
        ...
        self.occ_fuser = (
            builder.build_fusion_layer(occ_fuser) if occ_fuser is not None else None
        )
    ...
    def extract_feat(self, points, img, img_metas):
        ...
        if self.occ_fuser is not None:
            ...
            voxel_feats = (
                img_voxel_feats if pts_voxel_feats is None else pts_voxel_feats
            )
    ...
    def simple_test(
        ...
        if output["output_voxels_fine"] is not None:
            if output["output_coords_fine"] is not None:
                ...
                pred_f = (
                    self.empty_idx
                    * torch.ones_like(gt_occ)[:, None]
                    .repeat(1, fine_pred.shape[1], 1, 1, 1)
                    .float()
                )
                pred_f[
                    :, :, fine_coord[0], fine_coord[1], fine_coord[2]
                ] = fine_pred.permute(1, 0)[None]
            else:
                pred_f = output["output_voxels_fine"][0]
            SC_metric, _ = self.evaluation_semantic(
                pred_f, gt_occ, eval_type="SC", visible_mask=visible_mask
            )
            SSC_metric_fine, SSC_occ_metric_fine = self.evaluation_semantic(
    ...
    def evaluation_semantic(self, pred, gt, eval_type, visible_mask=None):
        _, H, W, D = gt.shape
        pred = F.interpolate(
            pred, size=[H, W, D], mode="trilinear", align_corners=False
        ).contiguous()
...
def fast_hist(pred, label, max_label=18):
    ...
    bin_count = np.bincount(
        max_label * label.astype(int) + pred, minlength=max_label**2
    )

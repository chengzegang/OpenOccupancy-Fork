...
from projects.occ_plugin.utils.nusc_param import (
...
class OccHead(nn.Module):
    def __init__(
        ...
        self.loss_voxel_lovasz_weight = self.loss_weight_cfg.get(
            "loss_voxel_lovasz_weight", 1.0
        )
        ...
        if balance_cls_weight:
            self.class_weights = torch.from_numpy(
                1 / np.log(nusc_class_frequencies + 0.001)
            )
        else:
    ...
    def forward_coarse_voxel(self, voxel_feats):
        ...
        if self.soft_weights:
            ...
            voxel_soft_weights = (
                torch.ones(
                    [output_occs[0].shape[0], self.num_point_sampling_feat, 1, 1, 1],
                ).to(output_occs[0].device)
                / self.num_point_sampling_feat
            )
        ...
        for feats, weights in zip(output_occs, torch.unbind(voxel_soft_weights, dim=1)):
            feats = F.interpolate(
                feats, size=[H, W, D], mode="trilinear", align_corners=False
            ).contiguous()
    ...
    def forward(
        self, voxel_feats, img_feats=None, pts_feats=None, transform=None, **kwargs
    ):
        ...
        if self.cascade_ratio != 1:
            if self.sample_from_img or self.sample_from_voxel:
                ...
                coarse_coord_x, coarse_coord_y, coarse_coord_z = torch.meshgrid(
                ...
                for b in range(B):
                    ...
                    if self.training:
                        this_fine_coord = coarse_to_fine_coordinates(
                            this_coarse_coord, self.cascade_ratio, topk=self.fine_topk
                        )  # 3, 8N/64N
                    else:
                        this_fine_coord = coarse_to_fine_coordinates(
                    ...
                    new_coord = (
                        this_fine_coord[None].permute(0, 2, 1).float().contiguous()
                    )  # x y z

                    if self.sample_from_voxel:
                        this_fine_coord = this_fine_coord.float()
                        this_fine_coord[0, :] = (
                            this_fine_coord[0, :] / (self.final_occ_size[0] - 1) - 0.5
                        ) * 2
                        this_fine_coord[1, :] = (
                            this_fine_coord[1, :] / (self.final_occ_size[1] - 1) - 0.5
                        ) * 2
                        this_fine_coord[2, :] = (
                            this_fine_coord[2, :] / (self.final_occ_size[2] - 1) - 0.5
                        ) * 2
                        this_fine_coord = (
                            this_fine_coord[None, None, None]
                            .permute(0, 4, 1, 2, 3)
                            .float()
                        )
                    ...
                    if img_feats is not None and self.sample_from_img:
                        W_new, H_new, D_new = (
                            W * self.cascade_ratio,
                            H * self.cascade_ratio,
                            D * self.cascade_ratio,
                        )
                        ...
                        for img_feat in img_feats:
                            sampled_img_feat = F.grid_sample(
                                img_feat[b].contiguous(),
                                img_uv.contiguous(),
                                align_corners=True,
                                mode="bilinear",
                                padding_mode="zeros",
                            ...
                            sampled_img_feat = (
                                sampled_img_feat * img_mask.permute(2, 1, 0)[:, None]
                            )
                            sampled_img_feat = self.img_mlp(
                                sampled_img_feat.sum(0)[:, :, 0].permute(1, 0)
                            )
    ...
    def loss_voxel(self, output_voxels, target_voxels, tag):
        ...
        if ratio != 1:
            target_voxels = (
                target_voxels.reshape(B, H, ratio, W, ratio, D, ratio)
                .permute(0, 1, 3, 5, 2, 4, 6)
                .reshape(B, H, W, D, ratio**3)
            )
            ...
            occ_space[occ_space == 0] = (
                -torch.arange(len(occ_space[occ_space == 0])).to(occ_space.device) - 1
            )
        ...
        loss_dict[
            "loss_voxel_sem_scal_{}".format(tag)
        ] = self.loss_voxel_sem_scal_weight * sem_scal_loss(
            output_voxels, target_voxels, ignore_index=255
        )
        loss_dict[
            "loss_voxel_geo_scal_{}".format(tag)
        ] = self.loss_voxel_geo_scal_weight * geo_scal_loss(
            output_voxels, target_voxels, ignore_index=255, non_empty_idx=self.empty_idx
        )
        loss_dict[
            "loss_voxel_lovasz_{}".format(tag)
    ...
    def loss_point(self, fine_coord, fine_output, target_voxels, tag):
        selected_gt = target_voxels[
            :, fine_coord[0, :], fine_coord[1, :], fine_coord[2, :]
        ].long()[0]
        assert torch.isnan(selected_gt).sum().item() == 0, (
            torch.isnan(selected_gt).sum().item()
        )
        assert torch.isnan(fine_output).sum().item() == 0, (
            torch.isnan(fine_output).sum().item()
        )
        ...
        loss_dict[
            ...
            fine_output, selected_gt, ignore_index=255
        )
        loss_dict[
            "loss_voxel_sem_scal_{}".format(tag)
        ] = self.loss_voxel_sem_scal_weight * sem_scal_loss(
        ...
        loss_dict[
            ...
            fine_output, selected_gt, ignore_index=255, non_empty_idx=self.empty_idx
        )
        loss_dict[
            "loss_voxel_lovasz_{}".format(tag)
        ] = self.loss_voxel_lovasz_weight * lovasz_softmax(
    ...
    def loss(
        ...
        **kwargs
        ...
        for index, output_voxel in enumerate(output_voxels):
            loss_dict.update(
                self.loss_voxel(output_voxel, target_voxels, tag="c_{}".format(index))
            )
        if self.cascade_ratio != 1:
            loss_batch_dict = {}
            if self.sample_from_voxel or self.sample_from_img:
                for index, (fine_coord, fine_output) in enumerate(
                    zip(output_coords_fine, output_voxels_fine)
                ):
                    this_batch_loss = self.loss_point(
                        fine_coord, fine_output, target_voxels, tag="fine"
                    )

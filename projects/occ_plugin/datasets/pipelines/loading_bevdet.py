...
class LoadMultiViewImageFromFiles_BEVDet(object):
    ...
    def get_sensor2lidar_transformation(self, cam_info, cam_name, sample_info):
        ...
        lidarego2global_rot = torch.Tensor(Quaternion(w, x, y, z).rotation_matrix)
        ...
        lidar2ego_rot = torch.Tensor(Quaternion(w, x, y, z).rotation_matrix)
        ...
        # camera to lidar
        sweepsensor2lidar = (
            ego2lidar @ global2lidarego @ sweepego2global @ sweepsensor2sweepego
        )
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam_name in cam_names:
            ...
            sensor2lidar = (
                torch.tensor(results["lidar2cam_dic"][cam_name]).inverse().float()
            )
            ...
            if self.load_depth:
                ...
                point_depth = np.fromfile(
                    os.path.join(self.depth_gt_path, f"{file_name}.bin"),
                    dtype=np.float32,
                    count=-1,
                ).reshape(-1, 3)
...
class LoadAnnotationsBEVDepth:
    def __init__(self, bda_aug_conf, classes, is_train=True, input_modality=None):
